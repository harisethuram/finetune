...
parser.add_argument("--train_cap", type=int, nargs='?', const=1000)
parser.add_argument("--val_cap", type=int, nargs='?', const=1000)
parser.add_argument("--test_cap", type=int, nargs='?', const=1000)
...
def main():
    ...
    print(f"batch size: {args.b_size}")
    ...
    test_dataset = raw_dataset['test'][:args.test_cap]

    print("Preprocessing data...")
    train_loader = DataLoader(preprocess(train_dataset, tokenizer, args.input_cols, args.null_lab, DEVICE), args.b_size)
    validation_loader = DataLoader(preprocess(validation_dataset, tokenizer, args.input_cols, args.null_lab, DEVICE), args.b_size)
    test_loader = DataLoader(preprocess(test_dataset, tokenizer, args.input_cols, args.null_lab, DEVICE), args.b_size)
    ...
    train_metrics, val_metrics, post_y_hat, losses = finetune(classification_model, loss_fn, args.num_epochs, optimizer, train_loader, validation_loader, args.num_labels, args.classification_idx)
